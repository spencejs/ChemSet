...
if (get_option('blogdescription') === 'Just another WordPress site') {
	add_action('admin_notices', create_function('', "echo '<div class=\"error\"><p>" . sprintf(__('Please update your <a href="%s">site tagline</a>', 'chemset'), admin_url('options-general.php')) . "</p></div>';"));
};

// check to see if htaccess is writable
// show an admin notice to update if it isn't
function chemset_htaccess_writable() {
	if (!is_writable(get_home_path() . '.htaccess')) {
		add_action('admin_notices', create_function('', "echo '<div class=\"error\"><p>" . sprintf(__('Please make sure your <a href="%s">.htaccess</a> file is writeable ', 'chemset'), admin_url('options-permalink.php')) . "</p></div>';"));
	};
}

add_action('admin_init', 'chemset_htaccess_writable');
...
function chemset_change_mce_options($options) {
...
add_filter('tiny_mce_before_init', 'chemset_change_mce_options');
...
function chemset_remove_dashboard_widgets() {
...
add_action('admin_init', 'chemset_remove_dashboard_widgets');
